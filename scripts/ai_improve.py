...
Same script as the other ssevera1 repos, with the file ranking tuned for a
Unity project: C#, Java and shader sources under unity-project/ and
companion-app/ are surveyed first, and Unity's generated files (.meta,
.unity, .asset) are never rewritten.
"""
...
PRIORITY_DIRS = {"unity-project", "companion-app", "tools", "scripts"}
PRIORITY_EXTS = {".cs", ".java", ".shader", ".py", ".sh"}
READABLE_EXTS = PRIORITY_EXTS | {".yaml", ".yml", ".toml", ".md", ".json", ".gradle", ".xml", ".asmdef"}
SKIP_DIRS = {
    ".git", "node_modules", "__pycache__", ".venv", "venv",
    "dist", "build", ".pytest_cache", ".mypy_cache", "coverage",
    "Library", "Temp", "Logs", "Obj", "Builds", "builds", ".gradle", "wrapper",
}
SKIP_FILES = {
    "package-lock.json", "yarn.lock", "pnpm-lock.yaml",
    "poetry.lock", "Pipfile.lock", ".gitignore", "packages-lock.json",
}
# Unity-generated or binary-ish files that must never be rewritten by a model.
BLOCKED_SUFFIXES = {".yml", ".yaml", ".lock", ".meta", ".unity", ".asset", ".prefab", ".jar", ".bat"}
...
codebase you know well. This is a Unity 2022.3 AR application (C#) with an \
Android companion app (Java). Unity scripts cannot be compiled in CI, so favour \
changes that are obviously correct from reading the code, and add an EditMode \
test under unity-project/Assets/Tests/EditMode when the change is testable.
...
- An unhandled edge case or error path (null/empty input, socket failure, \
malformed network payload, race between a socket thread and the Unity main thread)
- A missing test that covers observable, non-trivial behaviour
- A hardcoded value that belongs in a [SerializeField] or DSpaceConfig
- A missing log line at a decision point that would help debug on a headset
...
- Do NOT pick README.md, *.yml/*.yaml workflow files, lock files, or Unity \
generated files (*.meta, *.unity, *.asset)
...
- Unity C# must stay C# 9 compatible (Unity 2022.3); no file-scoped namespaces, \
no records with primary constructors, no required members
...
def main() -> None:
    ...
    # A brand-new asset under unity-project/Assets needs a .meta or Unity will
    # invent a GUID on import; generate it deterministically alongside.
    extra_paths = [str(file_path)]
    if not original and "unity-project" in file_path.parts:
        gen = repo_root / "tools" / "unity" / "generate_meta.py"
        if gen.is_file():
            subprocess.run([sys.executable, str(gen)], check=False)
            meta = Path(str(file_path) + ".meta")
            if meta.is_file():
                extra_paths.append(str(meta))
